...
def load_dialogues(directory):
    manifest = json.loads((directory / "manifest.json").read_text(encoding="utf-8"))
    path = directory / "test.jsonl"
    if hashlib.sha256(path.read_bytes()).hexdigest() != manifest["file_sha256"]["test.jsonl"]:
        raise ValueError("The test file differs from its dataset manifest.")
    rows = [json.loads(line) for line in path.read_text(encoding="utf-8").splitlines()]
    if len(rows) != manifest["split_counts"]["test"] or len({r["id"] for r in rows}) != len(rows):
        raise ValueError("Unexpected test count or duplicate IDs.")
    for row in rows:
