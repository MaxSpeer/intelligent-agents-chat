...
### Plain-English pilot and answer masking

The Plain-English dataset and its manually started v2 run are documented in
[`plain-english-v2.md`](plain-english-v2.md). The first run and its evaluation
remain preserved; v2 writes to a separate adapter directory.

The shared trainer now uses `chat_examples.py`: one example per assistant answer,
with earlier messages masked as context and a non-thinking Qwen3 prefix. Qwen3
formats historical and final assistant turns differently, so subtracting lengths
of separately rendered prefixes could previously supervise the beginning of the
next user message. The new code checks exact text and token-prefix alignment,
including the answer end marker, and rejects overlong examples. Multi-turn JSONL
rows now produce multiple training examples; recheck step-based save/eval cadence.
