#!/usr/bin/env python3
"""Train a Qwen3 LoRA adapter with answer-only supervision."""
...
from training.conspiracy.examples import Example, build_examples
...
OUTPUT_DIR = Path(os.environ.get(
    "TRAINING_OUTPUT_DIR",
    "/sc/projects/sci-lippert/intelligent-agents/project_matthias_max/adapters/qwen3-8b-conspiracy-training",
))
...
def main(*, tokenizer=None, callback_factory=None) -> None:
    ...
    if not TRAIN_FILE.is_file() or not VAL_FILE.is_file():
        raise FileNotFoundError("Prepare and augment the conspiracy dataset before training")
    set_seed(SEED)
