...
class CpuIntegrationTests(unittest.TestCase):
    def test_real_dataset_and_padded_labels(self):
        ...
        from training.simple_english.trainer import JsonlChatDataset, collate
        tok = AutoTokenizer.from_pretrained(os.environ["QWEN_TOKENIZER_PATH"], local_files_only=True)
        dataset = JsonlChatDataset(Path(__file__).resolve().parents[1] / "data/train.jsonl", tok, 1024)
        self.assertEqual(dataset.conversations, 2000)
        self.assertEqual(len(dataset), 3085)
    ...
    def test_installed_qwen_peft_generation_api_on_tiny_random_cpu_model(self):
        ...
        from training.simple_english.validation_samples import GENERATION_SETTINGS
