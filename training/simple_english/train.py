#!/usr/bin/env python3
"""Validate Simple English data; use --run to train in a Slurm GPU allocation."""
...
from training.simple_english.evaluate import load_dialogues, write_json
from training.simple_english.validate import validate


HERE = Path(__file__).resolve().parent
PROJECT = Path("/sc/projects/sci-lippert/intelligent-agents/project_matthias_max")
DATA = HERE / "data"
OUTPUT = PROJECT / "adapters" / "qwen3-8b-simple-english"
SETTINGS = {
...
CODE_FILES = ("train.py", "trainer.py", "examples.py", "validation_samples.py", "evaluate.py", "validate.py")
...
def preflight(tokenizer_path: Path, output_dir: Path):
    ...
    examples = sum(
        len(build_examples(tokenizer, row["messages"], SETTINGS["MAX_SEQ_LEN"]))
    ...
    sampled_validation_rows = [rows_by_id[identifier] for identifier in sample_ids]
    plan = {
        "status": "checked_not_started",
        "settings": SETTINGS,
        ...
        "train_answer_examples": examples,
        ...
        "validation_sampling": "fixed 10 dialogues: base neutral/style, then each saved epoch",
        "validation_generation_ids": sample_ids,
        "validation_loss_uses_full_split": True,
        "test_generations": False,
        "validation_report": report,
        "code_sha256": {
            name: hashlib.sha256((HERE / name).read_bytes()).hexdigest() for name in CODE_FILES
        },
        "packages": {
    ...
    return plan, tokenizer, sampled_validation_rows
...
def main():
    ...
    parser.add_argument(
        "--tokenizer", type=Path, default=Path(manifest["tokenizer"]["source_path"])
    )
    ...
    from training.simple_english import trainer as training
    from training.simple_english.validation_samples import ValidationSamplesCallback
    ...
    if free_bytes < 16 * 1024**3:
        raise SystemExit(
            "Less than 16 GiB GPU memory free. Use an available GPU; "
    ...
    for name in ("train.jsonl", "validation.jsonl", "manifest.json"):
        ...
        expected = plan["dataset_file_sha256"].get(name)
        if expected and hashlib.sha256((inputs / name).read_bytes()).hexdigest() != expected:
            raise ValueError(f"Dataset changed since preflight: {name}")
    ...
    try:
        training.main(
            ...
            callback_factory=lambda model, tok: [
                ValidationSamplesCallback(
                    model, tok, validation_rows, output_dir, seed=SETTINGS["SEED"]
                ),
            ],
        )
